...
describe('loadViewport', () => {
  ...
  it('is tolerant of a zoom outside [MIN_ZOOM, MAX_ZOOM], returning null so the caller falls back to fit', () => {
    const storage = memoryStorage({
      'assembly-workbench:viewport:file1': JSON.stringify({ x: 0, y: 0, zoom: MIN_ZOOM - 0.01 }),
      'assembly-workbench:viewport:file2': JSON.stringify({ x: 0, y: 0, zoom: MAX_ZOOM + 0.01 }),
    });
    expect(loadViewport(storage, 'file1')).toBeNull();
    expect(loadViewport(storage, 'file2')).toBeNull();
  });

  it('accepts a zoom exactly at MIN_ZOOM or MAX_ZOOM (the range is inclusive)', () => {
    const storage = memoryStorage({
      'assembly-workbench:viewport:file1': JSON.stringify({ x: 0, y: 0, zoom: MIN_ZOOM }),
      'assembly-workbench:viewport:file2': JSON.stringify({ x: 0, y: 0, zoom: MAX_ZOOM }),
    });
    expect(loadViewport(storage, 'file1')).toEqual({ x: 0, y: 0, zoom: MIN_ZOOM });
    expect(loadViewport(storage, 'file2')).toEqual({ x: 0, y: 0, zoom: MAX_ZOOM });
  });
  ...
  it('is tolerant of a zoom that overflows to Infinity from a huge JSON number literal, returning null', () => {
    ...
    const storage = memoryStorage({
      'assembly-workbench:viewport:file1': '{"x":0,"y":0,"zoom":1e400}',
    });
    expect(loadViewport(storage, 'file1')).toBeNull();
  });

  it('is tolerant of an x or y that overflows to Infinity from a huge JSON number literal, returning null', () => {
    const storageX = memoryStorage({ 'assembly-workbench:viewport:filex': '{"x":1e400,"y":0,"zoom":1}' });
    const storageY = memoryStorage({ 'assembly-workbench:viewport:filey': '{"x":0,"y":-1e400,"zoom":1}' });
    expect(loadViewport(storageX, 'filex')).toBeNull();
    expect(loadViewport(storageY, 'filey')).toBeNull();
  });
