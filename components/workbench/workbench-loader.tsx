...
export function WorkbenchLoader({
  ...
  // Workbench: initialLayout, the saver's fileId/initialUpdatedAt and the
  // name state are all seeded once from props via lazy useState, so reusing
  // the instance across files would keep showing the first file's content.
  return <Workbench key={file.id} file={file} layoutInvalid={layoutInvalid} />;
}
