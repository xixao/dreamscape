...
export function ElementPicker() {
  ...
  return <aside aria-label="Builder components" className="flex shrink-0 flex-col border-b border-line-soft pb-3">
    <h2 className={`${LABEL} flex items-center gap-2 px-4 pt-4 pb-3`}><Blocks className="size-4" />Components</h2>
    <input role="combobox" aria-label="Add a component" placeholder="Add a component…" aria-autocomplete="list" aria-expanded={true} aria-controls={listId}
    ...
    <div id={listId} role="listbox" aria-label="Components to add" className="max-h-44 overflow-y-auto px-2">
    ...
    {!results.length && <p role="status" className="px-4 py-2 text-xs text-muted-foreground">No matching components.</p>}
    {!query && <button onClick={() => { setBrowse(current => !current); setHighlight(0); }} className="mx-4 mt-2 self-start text-xs text-acc">{browse ? 'Show common components' : 'Browse all'}</button>}
  </aside>;
}
