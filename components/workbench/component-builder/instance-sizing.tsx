...
export function InstanceSizing({ props, onChange }: { props: Record<string, unknown>; onChange: (patch: Record<string, unknown>) => void }) {
  ...
  return <div className="flex flex-col gap-3">
    <div className="flex items-center gap-2 text-xs"><span className="flex-1">Width</span>
      {mode !== 'fill' && <input className={`${control} w-20 font-mono`} aria-label="Component width" type="number" min={1} value={Number(mode === 'percent' ? props.widthPercent ?? 100 : props.widthPx ?? 320)} onKeyDown={event => shiftNumericStep(event, Number(mode === 'percent' ? props.widthPercent ?? 100 : props.widthPx ?? 320), value => onChange({ [mode === 'percent' ? 'widthPercent' : 'widthPx']: value }), 1)} onChange={e => { const value = Number(e.target.value); if (value > 0) onChange({ [mode === 'percent' ? 'widthPercent' : 'widthPx']: value }); }} />}
    ...
    <div className="flex items-center gap-2 text-xs"><span className="flex-1">Max width</span>
      {limit && <input className={`${control} w-20 font-mono`} aria-label="Maximum width" type="number" min={1} value={limit.value} onKeyDown={event => shiftNumericStep(event, limit.value, value => onChange({ maxWidth: { ...limit, value } }), 1)} onChange={e => { const value = Number(e.target.value); if (value > 0) onChange({ maxWidth: { ...limit, value } }); }} />}
