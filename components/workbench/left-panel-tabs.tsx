...
export const LeftPanelContext = createContext<{ chatOpen: boolean; setChatOpen: (open: boolean) => void; collapsed?: boolean; setCollapsed?: (collapsed: boolean) => void } | null>(null);
export function LeftPanelTabs({ compact = false }: { compact?: boolean }) {
  const context = useContext(LeftPanelContext);
  if (!context) return null;
  return <ToggleGroup type="single" aria-label="Left panel mode" value={context.chatOpen ? 'chat' : 'layers'} onValueChange={value => { if (value) { context.setChatOpen(value === 'chat'); context.setCollapsed?.(false); } }} className={`${SEG_GROUP} ${compact ? 'flex-col' : 'flex-1'}`}>
