...
- Frame position in the file: each screen gains `x` and `y` (canvas px, integers; existing files get positions on first load and save them on the next change). A screen with no position yet - on first load, or a newly added/duplicated screen - is placed to the right of the rightmost already-positioned frame in the file (`max(x + stageWidth)` over positioned screens, plus a 200 px gap, at the `y` of the screen it chains off or 0), never overlapping another frame; several such screens in the same batch continue left to right from there. Stored in the screens JSON with `stageWidth`, `stageHeight`, `deviceName`.
