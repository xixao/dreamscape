...
- [ ] Show/Hide UI (Matt's request, 2026-09-12): Cmd+\ on Mac and Ctrl+\ elsewhere toggles the top bar, the Assets panel and the Design panel together, exactly like Figma's Show/Hide UI. State `uiHidden` lives in `WorkbenchShell` (not persisted). When hidden, the shell renders only the stage column filling the window (`grid-cols-[1fr] grid-rows-[1fr]`, same 12 px padding), the three panels are not rendered, and the stage re-measures its zoom (the existing `ResizeObserver` on the column covers it). The shortcut is handled in `useWorkbenchKeyboard` through a new option `onToggleUi` and fires even when the focus is in an input, a select or a dialog (it runs before the editable-target and popup guards); `preventDefault` so the browser does nothing with it. Tests in `keyboard.test.tsx` (Cmd+\ and Ctrl+\ call the callback, also from inside an input) and `workbench.test.tsx` (after Cmd+\ the Assets and Design panels and the top bar are gone and the artboard is still there; again brings them back).
