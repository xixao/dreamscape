...
Click the file name to open File settings, where designers can rename the file and choose External - Light, External - Dark, Internal - Light, or Internal - Dark. Select a root frame to set Appearance in the Design inspector: File default (with the inherited value) or any of the four themes. New frames inherit the file default. Explicit overrides survive changes to the file default.
...
External themes retain the legacy `light`/`dark` values. Internal themes use `internal-light`/`internal-dark`; they currently share the corresponding external color palette while preserving a distinct saved selection.
